...
inline vector<fst::CommonPrefixSearchResult<output_t>> prefix_match(
  const vector<char>& byte_code, const char* str)
{
  vector<fst::CommonPrefixSearchResult<output_t>> ret;
  fst::common_prefix_search<output_t>(byte_code.data(), byte_code.size(), str, [&](const auto& result) {
...
inline vector<output_t> exact_match_t(const vector<char>& byte_code, const char* str)
{
  vector<output_t> outputs;
  fst::exact_match_search<output_t>(byte_code.data(), byte_code.size(), str, [&](const output_t& val) {
    outputs.emplace_back(val);
  });
  return outputs;
}

inline vector<output_t> exact_match(const vector<char>& byte_code, const char* str)
{
...
TEST_CASE("Exact match search", "[readme]")
{
  ...
  REQUIRE(fst::exact_match_search<uint32_t>(t.data(), t.size(), "apr")[0] == 30);
  REQUIRE(fst::exact_match_search<uint32_t>(t.data(), t.size(), "ap").empty());
  REQUIRE(fst::exact_match_search<uint32_t>(t.data(), t.size(), "apr_").empty());
  REQUIRE(fst::exact_match_search<uint32_t>(t.data(), t.size(), "feb")[0] == 28);
  REQUIRE(fst::exact_match_search<uint32_t>(t.data(), t.size(), "feb")[1] == 29);
}
